...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # Note: W1 is (H, D), so we transpose it to match X (N, D)
    h1 = np.maximum(0, X.dot(W1.T) + b1)
    
    # Second layer: Affine (Output layer, no ReLU)
    # Note: W2 is (C, H), so we transpose it
    ...
    # Compute Softmax loss (Data loss)
    ...
    # Backward pass: compute gradient of scores
    dscores = probs
    dscores[np.arange(N), y] -= 1
    dscores /= N

    # Backprop into W2 and b2
    # W2 gradient: (dscores^T * h1) + regularization
    # Note: We need to match the shape of W2 (C, H)
    grads['W2'] = dscores.T.dot(h1) + reg * W2
    ...
    # W1 gradient: (dh1^T * X) + regularization
    # Note: We need to match the shape of W1 (H, D)
  ...
  def predict(self, X):
    ...
    # ================================================================ #
    h1 = np.maximum(0, X.dot(self.params['W1'].T) + self.params['b1'])

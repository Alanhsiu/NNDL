...
def affine_forward(x, w, b):
  ...
  N = x.shape[0]
  # Reshape x to (N, D) then dot product with w (D, M) plus b (M,)
...
def affine_backward(dout, cache):
  ...
  N = x.shape[0]
  # dx: dout * w.T -> reshape back to input shape
  dx = dout.dot(w.T).reshape(x.shape)
  # dw: x.T * dout -> reshape x to (N, D) first
  dw = x.reshape(N, -1).T.dot(dout)
  # db: sum of dout across batch dimension
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    # (1) Calculate the sample mean and variance of the minibatch
    mu = np.mean(x, axis=0)
    var = np.var(x, axis=0)
    ...
    # (4) Store variables in cache for the backward pass
    ...
    # Normalize using the running mean and variance during test time
...
def batchnorm_backward(dout, cache):
  ...
  # Step-by-step backprop through the batchnorm computation graph
  dx_hat = dout * gamma
  ...
  dmu = np.sum(dx_hat * -1.0 / np.sqrt(var + eps), axis=0) + dvar * np.sum(-2.0 * (x - mu)) / N
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    # Create a dropout mask, retaining each neuron with probability p.
    # We divide by p here (Inverted Dropout) so we don't have to scale at test time.
    ...
    # During test time, inverted dropout just passes the input through
    out = x
...
def dropout_backward(dout, cache):
  ...
  if mode == 'train':
    ...
    # During test time, the gradient simply passes through
    dx = dout

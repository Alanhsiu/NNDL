...
def sgd_nesterov_momentum(w, dw, config=None):
    ...
    # Apply the Nesterov momentum update rule (lookahead adjustment)
...
def rmsprop(w, dw, config=None):
    ...
    # Update the moving average of the squared gradients (second moment)
    config['a'] = config['decay_rate'] * config['a'] + (1 - config['decay_rate']) * (dw ** 2)
    
    # Update the weights by dividing the learning rate by the square root of the moving average
...
def adam(w, dw, config=None):
    ...
    # Update the moving average of the first moment (gradient)
    config['v'] = config['beta1'] * config['v'] + (1 - config['beta1']) * dw
    
    # Update the moving average of the second moment (squared gradient)
    config['a'] = config['beta2'] * config['a'] + (1 - config['beta2']) * (dw ** 2)
    
    # Compute bias-corrected first and second moments to mitigate initialization bias
